...
More information can be found in the [heading attrs spec page](https://github.com/raphlinus/pulldown-cmark/blob/master/specs/heading_attrs.txt)."#####;
    ...
    fn no_change_round_trip_preprocessor_run() {
        // manually written content to pass test
    ...
    /// These tests fail because the pulldown-cmark loses information during parsing.
    #[test]
    fn round_trip_preprocessor_fail_components() {
        let ctx: PreprocessorContext = mock_context();

        // weird behaviour. backticks added etc. doesn't seem to happen when more content is inserted.
        let content = "# Markdown\n\n     towards";
        let book: Book = mock_book(content);
        let expected_book = book.clone();
        let result = Classy::new().run(&ctx, book);
        ...
        // white space is stripped in a new line
        let content = "# Markdown\n\nmdBook's [parser](https://github.com/raphlinus/pulldown-cmark)\n      towards";
        let book: Book = mock_book(content);
        let expected_book = book.clone();
        let result = Classy::new().run(&ctx, book);
        assert!(result.is_ok());
        ...
        // white space is stripped between > and text
        let content = "# Markdown\n\nmdBook's [parser](https://github.com/raphlinus/pulldown-cmark)\n >      towards";
        let book: Book = mock_book(content);
        let expected_book = book.clone();
        let result = Classy::new().run(&ctx, book);
        ...
        // TODO: this adds in a tab for the > for some reason. Possibly due to quote block
        let content = "# Markdown\n\n > \n > [^note]: This text is the contents of the footnote, which will be rendered\n > towards the bottom.\n\n";
        let book: Book = mock_book(content);
        let expected_book = book.clone();
        let result = Classy::new().run(&ctx, book);
